...
double bodyforcey = 0;    // Magnitude of body force acting on the droplet in the x direction
...
double initFluid(const int k) {
    ...
    // Smooth droplet
    return (0.5 - 0.5 * tanh(2 * (sqrt(rr2) - radius) / (6.0))) * // Circle with given radius
           (0.5 + 0.5 * tanh(2 * (yy - postheight) / (6.0)));
    // Sharp droplet
...
void initParams(std::string inputfile) {
    ...
    params.addParameter<double>(bodyforcey, "bodyforcey");
...
using DefaultTraitPressureWellBalancedNew = typename DefaultTrait<TLattice,2>::template SetBoundary<BounceBack, FreeSlip>
                                                               ...
                                                               :: template SetForce< PressureForceWellBalanced<WellBalancedForce<GuoPrefactor>,ChemicalForceBinary>, BodyForce<> >
...
auto initPressure() {
    ...
    FlowFieldPressureWellBalanced3<Lattice, DefaultTraitPressureWellBalancedNew<Lattice>> pressure;
...
using DefaultTraitWellBalancedCH2 = typename DefaultTrait<TLattice,Nmax>::template SetBoundary<BounceBack, FreeSlip>
                                                                ::template SetProcessor<std::tuple<LinearWetting,MirrorBoundary<OrderParameter<>>>,std::tuple<orderparamgradients, ChemicalPotentialCalculatorBinaryLee>,std::tuple<MirrorBoundary<ChemicalPotential<>>,MirrorBoundary<ChemicalPotential2<>>>> // Note the inclusion of a class to calculate the chemical potential
                                                               :: template SetForce< NoSource<0> >;
...
auto initBinary() {
    ...
    WellBalancedCH<0,2,Lattice, DefaultTraitWellBalancedCH2<0,2,Lattice>::template AddProcessor<std::tuple<GradientsMultiStencil<ChemicalPotential<0>,CentralQBounceBack,CentralXYZBounceBack>,Gradients<ChemicalPotential2<0>,CentralQFourthOrderBounceBack>>>> binary;
    ...
    binary.template getProcessor<ChemicalPotentialCalculatorBinaryLee>().setOmega(10);
    ...
    binary.template getProcessor<MirrorBoundary<ChemicalPotential2<>>>().setNodeID(2);

    double wettingprefactor = -2 * cos(90 * M_PI / 180.0) * sqrt(2 * A / kappa);
...
void AfterEquilibration(int eqsteps, T& model) {
    if (eqsteps == equilibriumtimesteps) model.template getForce<BodyForce<>>().setForce({bodyforcex, bodyforcey}, // bodyforcex in the x direction, 0 in the y direction
                                                                                         0); // Act on 0 component (droplet)
}

template <typename T>
void StopForce(int eqsteps, T& model) {
    if (eqsteps == equilibriumtimesteps+150) model.template getForce<BodyForce<>>().setForce({0,0}, // bodyforcex in the x direction, 0 in the y direction
                                                                                         0); // Act on 0 component (droplet)
}
