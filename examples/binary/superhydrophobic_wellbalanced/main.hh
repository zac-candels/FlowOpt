...
double R2 = 400.0;
double delta = 20.0; // Spacing between left and right curves
double left_foot = 100;
double center_circ1 = left_foot + sqrt(R2);
double right_foot = left_foot + delta;
double intersection = left_foot + sqrt(R2) + 0.5*delta;

double boundaryCurveFn(double x)
{
    return 5*sin(M_PI/50 * x) + 10;
}

double f_upper(double x, double center_circ1)
{
    return sqrt( R2 - pow((x - center_circ1),2) );
}

double f_lower(double x, double center_circ1, double delta)
{
    return sqrt(  R2 - pow( x - center_circ1 - delta, 2 )  );
}

double stepFunction(int xx, int yy)
{
    double x = (double)xx;
    double y = (double)yy;

    if ( (x >= left_foot) && (x <= right_foot) )
    {
        if( y < f_upper(x, center_circ1) )
        {
            return 0;
        }
        else
    ...
    else if ( (x >= right_foot) && (x <= intersection ) )
    {
        if ( (y < f_upper(x, center_circ1)) && (y > f_lower(x, center_circ1, delta)) )
        {
            return 0;
        }
        else
...
double stepFunction1(int xx, int yy)
{
    if ( (double)yy < f_upper( (double)xx, center_circ1) )
    {
        return 0;
    }
    else
...
int initBoundary(const int k) {
    ...
    // if ( (xx < left_end_pt_curve) || (xx > right_end_pt_curve) )
    // {
    //     if (yy <= 3) return 1;
    // }
    // else if ( (xx >= left_end_pt_curve)  )
    // {
    //     if (yy <= f_upper( (double)xx, left_end_pt_curve ) ) return 1;
    // }

    if ( (xx > left_foot) && (xx <= right_foot ) )
    {
        if( yy < f_upper(xx, center_circ1) )
        {
            return 1;
        }
    }
    else if ( (xx >= right_foot) && (xx <= intersection ) )
    {
        if ( (yy < f_upper(xx, center_circ1)) && (yy > f_lower(xx, center_circ1, delta)) )
        {
            return 1;
        }
        else if (yy <= 3)
    ...
    if ( (xx <= left_foot) || (xx >= intersection) )
    {
        if (yy <= 3) return 1;
    }
...
double initFluid(const int k) {
    ...
    // Smooth droplet
    return (0.5 - 0.5 * tanh(2 * (sqrt(rr2) - radius) / (4.0)))*stepFunction(xx, yy); // Defines a circle
...
auto initBinary() {
    ...
    double wettingprefactor = -2 * cos(45 * M_PI / 180.0) * sqrt(2 * A / kappa);
