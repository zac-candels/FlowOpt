...
// This script simulates two-phase capillary-dominated displacement through a porous structure in 2D.
// This example shows how to:
//      (i) pass the geometry to the lattice,
//      (ii) use HaoCheng and Convective BCs for the phase field lattice, and
//      (iii) define a region with no wettability to help with the stability of the displacement.
...
double initFluid(int k) {
    ...
    return tanh((x - lx / 10.) / (sqrt(2 * binaryKappa / binaryA)));
}
...
int main(int argc, char **argv) {
    ...
    using TraitFlowFieldBinary = DefaultTraitFlowFieldBinary<Lattice>::SetCollisionOperator<MRT>::SetBoundary<
        std::tuple<BounceBack>, std::tuple<VelocityInflow, Convective>>;
    using TraitBinary = DefaultTraitBinary<Lattice>::template SetProcessor<
        ...
        SetBoundary<std::tuple<BounceBack>, std::tuple<HaoCheng, Convective>>;
    ...
    componentSeparationModel.getBoundary<HaoCheng>().setNodeID(3);
    componentSeparationModel.getBoundary<HaoCheng>().setPhi(inletOrderParameter);
