...
def compute_numerical_capillary_rise(phi, it  = -1):
    ...
    y_2_index = np.where(np.abs(phi[it, int(phi.shape[1] / 2), 2:-2, 0]) < tol)[0][0] + 2
    y_2 = y_2_index
    h_num = y_2 - y_1
...
def compute_analytical_capillary_rise(lx, A, theta, rhog):
    ...
    h_ana = IFT * np.cos(np.deg2rad(theta)) / (r * rhog)
...
h_ana = compute_analytical_capillary_rise(lx = 200, A = 1.00E-03, theta = 30, rhog = 2.5E-06)
