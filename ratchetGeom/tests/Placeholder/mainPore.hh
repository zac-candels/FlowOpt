...
using DefaultTraitPressureLeeHumidityInflow = typename DefaultTraitPressureLee<TLattice>::template AddPostProcessor<
    Swapper<Velocity<>, VelocityOld<>, TLattice::NDIM>>::
    template AddForce<EvaporationPressureSource<EvaporationSourceMethod>>::template SetBoundary<
        PressureOutflow, BounceBack>;  //,std::tuple<Temp0Outflow>>>;,ExtrapolationOutflow
...
using DefaultTraitBinaryLeeHumidityInflow = typename DefaultTraitBinaryLee<TLattice>::template SetPostProcessor<
    ...
    ::template AddPreProcessor<GradientsMultiStencil<Pressure<>, CentralXYZBounceBack, CentralQBounceBack,
        ...
        template AddForce<EvaporationPhaseSource<EvaporationSourceMethod>>::template SetBoundary<ExtrapolationOutflow,
                                                                                                 BounceBack>;
...
void AfterEquilibration(int eqsteps, T& model) {
    if (eqsteps > equilibriumtimesteps)
        model.template getPreProcessor<MassLossCalculatorInterpolated<>>().setDiffusivity(diffusivity);
}
