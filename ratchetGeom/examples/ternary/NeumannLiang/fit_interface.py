...
def fit_circle(data, level=0, params_init=None):
    ...
    res = scipy.optimize.minimize(circle_eval, params_init, points, method="BFGS")
    x0, y0, k0 = res.x
    r0 = 1 / k0
    return x0, y0, r0
...
def measure_angle2d(order_param, solid=None, plane=None, plot=False, guess=None, mask_level=0.1):
    ...
    Guess = guess if (guess is not None) else [1000,400,30]
    xc, yc, rad = fit_circle(order_param, 0.5, Guess)#600+round(np.average(order_param)*1000*5)
    r0, normal = fit_plane(solid, 0.5) if (use_solid) else plane
    centre_height = np.sqrt(((xc)*normal[0] + (yc)*normal[1] - r0)**2)
    contact_angle = np.arccos(np.clip(-centre_height / rad, -1, 1)) * 180 / np.pi
    #print(600+round(np.average(order_param)*1000*5))
    #print(xc,yc,rad,centre_height,r0)
    #print(centre_height,rad)
    # Plot fitting?
    if (plot):
        ...
        plt.plot(xcirc, ycirc, lw=0.2, c='k')
        plt.plot(xplane, yplane, lw=0.2, c='k')

        points_circ = interface_points(order_param, 0.5)
        ...
        #plt.show()
        plt.savefig("CA.png", dpi=400, format='png')

    return contact_angle,xc,yc,rad
