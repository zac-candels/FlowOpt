...
def read_data(direc='data'):
    ...
    dens = np.zeros((len(times), lx, ly, lz))

    for it, t in enumerate(times):
        try:
            vel_file = open(direc+"/Velocity_t%li.mat"%t, 'rb')
            dens_file = open(direc+"/Density_t%li.mat"%t, 'rb')
            for k in range(lx*ly*lz):
                (xk,yk,zk) = coord_k(k,ly,lz)
                dens[it,xk,yk,zk] = struct.unpack('=d', dens_file.read(8))[0]
                for i in range(ndim):
...
def plotDens(dens):
    ...
    #ax.set_aspect('equal')
    #ax[1].set_ylabel('Density profile')
    for i in range(len(dens)):
        fig, ax = plt.subplots(1, 1)
        dens2d = dens[i,:,0,0]
        print(dens2d)
        #ax[0].contourf(dens2d.T, cmap='Reds')
        points, = ax.plot(dens2d)
        #plt.pause(0.5)
        #if (i<len(vel)-1): points.remove()
        plt.ylim(0.0025,0.0205)

        plt.savefig("dens"+str(i)+".png",format="png")
        plt.close()
    #plt.show()
    


dens,vel = read_data('data')
print([dens[-1,i,0,0] for i in range(1001)])
#plotDens(dens)
