...
for t in range(tstart, tend + 1, tinc):
    ...
    file_name = datadir + "Density_t%li.mat"%t
    ...
    print("average v: ",np.sum(liquid*v[:,:,:,1])/(np.sum(liquid)))
    print("min rho: ",np.amin(liquid))
    print(file_name)
    ...
    rgbdiss[:, :] = np.flip(np.sqrt(v[:,:,:,0]**2+v[:,:,:,1]**2)).transpose()
    
    # Visualise the array
    im = ax[0].imshow(rgbv,interpolation='nearest',origin='upper')
    im2 = ax[1].imshow(rgbdiss*500,interpolation='nearest',origin='upper',vmin=0,vmax=0.004*500)
    ax[0].contour(rgbv, levels=[17.3472916667], colors='black', linewidths=0.5)
    ax[1].contour(rgbv, levels=[17.3472916667], colors='black', linewidths=0.5)
    ax[1].set_title("Velocity Magnitude (m/s)")
    ax[0].set_title("Density")
    ax[0].set_xticks(np.round(np.linspace(0, LX, 6),1))
    ax[0].set_xticklabels(np.round(np.linspace(0, 2, 6),1))
    ax[0].set_yticks(np.round(np.linspace(LY, 0, 5),1))
    ax[0].set_yticklabels(np.round(np.linspace(0, 1.2, 5),1))
    ax[0].set_ylabel("y (mm)")
    ax[1].set_xticks(np.round(np.linspace(0, LX, 6),1))
    ax[1].set_xticklabels(np.round(np.linspace(0, 2, 6),1))
    ax[1].set_yticks(np.round(np.linspace(LY, 0, 5),1))
    ax[1].set_yticklabels(np.round(np.linspace(0, 1.2, 5),1))
    plt.tight_layout()
    stepx = 1
    stepy = 1
