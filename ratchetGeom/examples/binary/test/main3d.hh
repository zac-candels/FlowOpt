...
void initParams(std::string inputfile) {
    ...
    */
    ...
    x_pos_first_row = reservoir_length + 10;
...
int initBoundary(const int k) {
    ...
    // {
    //     return 1; // Gives us the constraining plate on the right side of the domain
    // }
    ...
    // {
    //     return 2; // Gives us solid (horizontal) walls on the top and bottom of the left reservoir
    // }

    // if( ( ( x >= lx - (reservoir_length )  ) && ( y <= 2 ) ) || ( ( x >= lx - (reservoir_length-2) ) && ( y >= ly - 2) ) )
    // {
    //     return 1; // Gives us solid (horizontal) walls on the top and bottom of the right reservoir
    // }
...
double initFluid(const int k) {
    ...
    double posx1 = (double)lx/4;
    double posy = (double)3;
    double posz = lz/2;
    double posx2 = (double)(lx - lx/4);

    // Radial distance from the centre of the droplet at (posx, posy)
    //double rr2 = (xx - posx) * (xx - posx) + (yy - posy) * (yy - posy);
    double dist1 = (xx - posx1) * (xx - posx1) + (yy - posy) * (yy - posy) + (zz - posz) * (zz - posz); // Switch these for 3D
    double dist2 = (xx - posx2) * (xx - posx2) + (yy - posy) * (yy - posy) + (zz - posz) * (zz - posz);
    ...
    if( xx <= lx/2 )
    {
        ...
        return 0.5 - 0.5*tanh(2*( sqrt(dist1) - radius)/4.0);
    }

    if( xx > lx/2 )
    {
        return 0.5 - 0.5*tanh(2*( sqrt(dist2) - radius)/4.0);
    }
...
auto initBinary() {
    ...
    //binary.template getProcessor<orderparamgradients>().setWettingPrefactor( wettingprefactor2 );
