...
for t in range(tstart, tend + 1, tinc):
    ...
    orderParam = np.zeros((LY, LX))
    rgbdiss = np.zeros((LY, LX))
    orderParam[:, :] = np.flip(liquid[:, :, zslice]).transpose()
    orderParam = np.fliplr(orderParam)
    ...
    # Visualise the array
    im = ax.imshow(orderParam, interpolation='nearest',origin='upper')
